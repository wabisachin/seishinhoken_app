...
const CASE_FRAMING_INSTRUCTION =
  "匿名の専門職またはクライエントを主人公にした短い場面をcase_textに書くこと。登場人物にはA・B・C…と"
  + "アルファベットを振るが、場面に登場する人物が1人なら「A精神保健福祉士」または「Aさん」のように"
  + "その1人だけをAとする。専門職とクライエントの両方が登場する場合は、必ず別々の文字を割り当てる"
  + "（例:「Aさん」（クライエント）と「B精神保健福祉士」、または「Aさん」と「B精神保健福祉士（以下「Bワーカー」という）」）。"
  + "同じ場面に登場する別人に同じ文字を使い、読み手が誰が誰だか混同する書き方は絶対にしないこと"
  + "（実際の過去問でもこの命名規則が一貫して守られている）。場面設定は根拠テキストの範囲で無理なく"
  + "成立させ、根拠テキストに無い事実は使わない。";
...
- 選択肢の作り方（最重要。過去問132問（正答が判明している設問）の全数精査に基づく。実際の
  ...
  - 誤答は必ず、根拠テキストまたは一般知識に基づく実在の別の事実に差し替えて作ること
    ...
    e. 「〜に含まれるものを選べ」「〜として適切なものを選べ」のような範囲・分類を問う問題
       （択二形式に特に多い）では、不正解の選択肢も出題対象と同じ広い分野・隣接領域から選ぶこと。
       正解の選択肢どうしにだけ共通のキーワード（例:「教育」）が入っていて、不正解の選択肢には
       そのキーワードが一切登場しない、というような作り方は、専門知識が無くても選択肢の並びを
       見ただけで正解が絞り込めてしまうため禁止。不正解も「一見それらしいが、定義上は対象に
       含まれない」ものにすること
  - 明らかに的外れな選択肢は禁止。受験者が迷う「近いが違う」ものにする（上記eの理由と同じ）
