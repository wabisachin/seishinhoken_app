...
- 文体による正答の露呈を絶対に避けること（最重要・頻発する失敗）:
  内容を理解していない受験者が、文章の「違和感」や「断定口調」だけで消去法により正答に
  たどり着けてしまう問題は、知識を問う問題として失格である。具体的には:
  - 5つの選択肢は、具体性・専門用語の密度・文の長さ・自信の度合いをすべて揃えて書くこと。
    正答だけが自然で誤答だけが不自然に極端・曖昧・簡潔、という差を作らない
  - 「必ず」「絶対に」「〜のみ」「一切〜ない」のような、いかにも誤りらしく響く極端な言い回しを
    誤答の目印として多用しない。本物の過去問の誤答の多くは、正答と同じくらい具体的で
    もっともらしい専門的記述であり、実際に知識と照合しないと誤りだと判断できない
