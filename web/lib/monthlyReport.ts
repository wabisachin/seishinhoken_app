...
const MistakeAnalysisSchema = z.object({
  ...
  fundamentalIssues: z
    ...
    .describe("複数の誤答に共通する、本人の本質的な課題を数学的帰納法的に（個別事例→一般化）導き出したもの"),
});
...
const NarrativeSchema = z.object({
  ...
  focusAreas: z.array(z.object({ subject: z.string(), why: z.string() })).describe("次月に重点的に取り組むべき科目とその理由"),
  planNarration: z.string().describe("次月の数値プラン（別途渡す）について、なぜこの量・この配分なのかを意味づけて説明する。数値そのものは新たに考案せず、渡された数値に言及するだけにすること"),
});
...
上記を踏まえ、前向きな導入・良かった点・弱点の噛み砕いた説明・重点科目とその理由・プランの意味づけを書いてください。`;
}
