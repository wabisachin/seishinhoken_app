...
const NarrativeSchema = z.object({
  ...
  planNarration: z
    .string()
    .describe(
      "次月の数値プラン（別途渡す）について、なぜこの量・この配分なのかを全体的な考え方として" +
        "意味づけて説明する。個別の科目名と問題数を列挙しないこと（例:「A科目は復習5問・演習10問」" +
        "のような科目ごとの内訳は、この文章の下に表として別途表示されるため、ここで重複して" +
        "書くと冗長で読みにくくなる）。代わりに『復習ストックが多い科目を優先した』『まだ手薄な" +
        "科目には演習を厚めにした』のような、配分の背景にある考え方だけを2〜3文で説明すること。" +
        "数値そのものは新たに考案せず、渡された数値の意味づけだけをすること。出題は科目単位の" +
...
# これ自体は変更せず、意味づけの文章だけを書くこと。この一覧は下に表として画面表示されるため、
# planNarrationで科目名・問題数を再度列挙しないこと）
