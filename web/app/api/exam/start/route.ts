...
export async function POST(req: NextRequest) {
  try {
    const { part } = (await req.json()) as { part?: ExamPart };
    ...
    let current: ExamAttemptRow | null = await getCurrentExamAttempt("self");
    ...
    const questionIds = await reserveQuestionsForPart(part);
    if (!questionIds) {
      after(() =>
