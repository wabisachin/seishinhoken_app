...
### 出題と生成の非同期化（かんばん方式のストック補充）

ユーザーが問題を解くスピードはその場生成より速いため、上記のリクエスト駆動だけでは
「次の問題へ」のたびに生成待ちが発生してしまう。そこで **科目ごとに「本人がまだ一度も
出題されていないactive問題」を常時5問（`STOCK_TARGET`）だけ確保しておく**トヨタの
かんばん方式的な補充を追加している（`web/lib/questionSupply.ts`の`topUpSubject`）。

これは前述の「常駐ループ」とは別物で、以下の2つの**呼ばれた分しか動かない**トリガーの
組み合わせで実現している。常駐プロセスは一切無く、上のコスト上限にも必ず従うため、
無限に生成され続けることは無い。

- **1日1回のVercel Cron**（`/api/cron/topup`）: 全科目のストックを数え直し、
  5問未満の科目だけを対象に、時間予算（270秒）の範囲内で並列に補充する
  （`CRON_SECRET`で保護、無関係な第三者が叩いて課金させることを防ぐ）。処理順は
  科目リストの並び順ではなく、**不足分（5問との差）に比例した重み付き抽選**
  （`topUpAllSubjects`の`weightedShuffle`）で決める。単純な降順ソートにしていない
  のは、却下が続いて時間を食う科目が毎回必ず先頭に来て他科目の順番を恒常的に
  奪う、という固定化を避けるため（確率的に不足が大きい科目を優先しつつ、
  ある程度のばらつきを残す）
- **出題直後のフック**（`/api/quiz/next`、Next.jsの`after()`）: ストックの「消費」は
  回答送信時ではなく問題が画面に出される（取り出される）タイミングで起きたとみなし、
  1問出題するたびにその科目のストックを非ブロッキングで確認・補充する。`after()`は
  レスポンス返却後に実行されるため、ユーザーの待ち時間には影響しない
- **デプロイ直後のコールドスタートフック**（`web/instrumentation.ts`の`register()`）:
  1日1回のCronを待たずに、新しいデプロイが本番に出た直後の最初のリクエストで
  全科目のストック補充を1回走らせる。Next.js標準のinstrumentation hookで、サーバー
  インスタンスが起動するたびに（＝新デプロイ後の最初のコールドスタートで）呼ばれる。
  デプロイごとに1回だけ実行されるよう、`app_settings.last_topup_deployment_id`との
  比較（`claimDeploymentTopUp`）で判定する（Vercelの負荷分散で複数インスタンスが
  同時にコールドスタートしても、DBの条件付き更新に勝った1インスタンスだけが実行）

`topUpSubject`はCronでも出題直後フックでも、**回数ではなく経過時間で自発的に
打ち切る**（`timeBudgetMs`）。1回の呼び出しで生成できる回数を固定してしまうと、
却下が続いた時に「1回も在庫を増やせないまま終わる」ことになり、ユーザーが1日に
何問も解く前提の下では実質的にストックが枯渇していく。時間で打ち切ることで、
却下が続いても予算の許す限り5問到達を狙い続けられる。ホストするルートの
`maxDuration`より確実に短い予算を渡すことが必須（そうしないとVercelに強制終了され、
`topUpInFlight`の解除（finally節）が走らずその科目が以後スキップされ続けるバグに
なる。実際にこれで起きたことがある）。

どちらも「未出題ストックが目標未満なら埋める」という同じ判定・同じ上限を使うだけの
冪等な処理なので、Cronが1日1回しか回らないVercel Hobbyプランでも、当日中は
回答フックが、翌日以降はCronが、それぞれ独立してストックを埋め合わせる。
ユーザーが1日に多数の問題を解く前提であれば、出題直後フックの方が実質的な
補充機会（トリガー回数）はCronよりずっと多い。
...
- **出題時（`getOrGenerateNext`）**: 未出題の在庫が1問でもあれば必ずそれを即座に返す。
  その場ライブ生成（ユーザーを待たせる）は、在庫が本当に尽きている場合の最終手段のみ
- **在庫の補充（`topUpSubject`）**: 「50問に到達するまでは必ず新規、200問に向けて
  徐々に新規率を下げる」という元々のコスト上限ポリシー自体は変わらない。生成タイミングが
  同期（即時）から非同期（このストック補充）に移っただけで、成熟した科目（50〜200問）
  では確率で「今回は生成しない」が選ばれることもあり、その場合はストックが目標の5問に
  届いていなくても無理に埋めない
