...
- **出題直後のフック**（`/api/quiz/next`、Next.jsの`after()`）: ストックの「消費」は
  ...
  レスポンス返却後に実行されるため、ユーザーの待ち時間には影響しない。ただし`after()`も
  そのルートの`maxDuration`を超えると強制終了され、その場合`topUpInFlight`の解除
  （finally節）が走らずその科目が以後スキップされ続けるバグになるため、
  こちらは**1回1問だけ**に絞っている（5問まで埋めきる本体の仕事はCronに任せる）
...
ただしアプリ立ち上げ直後などストックがどの科目もゼロに近い状態では、1日1回・
数分間のCronだけで18科目全部を1晩で満たすには足りず、数日かけて埋まっていく
（却下が続く科目はその分さらに時間がかかる）。
